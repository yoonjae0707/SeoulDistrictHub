...
async function bootstrapApp() {
  try {
    // [임시 헬퍼] 브라우저를 통해 깨진 지도를 복구하여 서버로 전송
    try {
      const mapCheck = await fetch("/src/assets/seoul_municipalities_geo_simple.json?t=" + Date.now());
      let needSave = false;
      if (!mapCheck.ok) {
        needSave = true;
      } else {
        const text = await mapCheck.text();
        if (text.length < 120000) { // 원본은 약 150KB 이상이나, 깨진 데이터는 17KB 또는 50KB 수준
          needSave = true;
        } else {
          try {
            const parsed = JSON.parse(text);
            if (!parsed.features || parsed.features.length < 25) {
              needSave = true;
            }
          } catch (e) {
            needSave = true;
      ...
      if (needSave) {
        console.log("Map file is missing or corrupted. Fetching fresh map from Github...");
      ...
      console.warn("Map auto-save proxy helper warning:", e);
    }

    // 1. Python 백엔드 API에서 자치구 데이터 가져오기
    const response = await fetch("/api/districts");
    if (!response.ok) {
      throw new Error("자치구 데이터를 불러오는 데 실패했습니다.");
    }
