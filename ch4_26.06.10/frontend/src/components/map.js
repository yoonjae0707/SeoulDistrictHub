...
export async function initMap(containerId, districtsData, onSelectDistrict) {
  ...
  // 1. Try to load local official simplified GeoJSON
  try {
    const response = await fetch("/src/assets/seoul_municipalities_geo_simple.json?t=" + Date.now());
    if (response.ok) {
      const localData = await response.json();
      if (localData && localData.features && localData.features.length === 25) {
        geojson = localData;
        console.log("Successfully loaded local Seoul GeoJSON map.");
      } else {
        console.warn("Local map data is incomplete or corrupted.");
      }
    } else {
      console.warn("Local map file not found or failed to load.");
    }
  } catch (err) {
    console.warn("Failed to read local GeoJSON:", err);
  }

  // 2. Fallback to Github if local fails or is corrupted
  if (!geojson) {
    try {
      console.log("Fetching fresh Seoul GeoJSON from Github...");
      ...
      if (response.ok) {
        ...
        if (githubData && githubData.features && githubData.features.length === 25) {
          geojson = githubData;
          loadedFromGithub = true;
          console.log("Successfully loaded Seoul GeoJSON from Github.");
        }
      }
    } catch (githubErr) {
      console.error("Failed to load map from Github:", githubErr);
    }
  }

  // 3. Render geographic map if we have complete GeoJSON, else fallback to hexagonal grid
  if (geojson) {
    try {
      renderGeographicMap(container, geojson, districtsData, onSelectDistrict);

      // If we loaded it from Github, trigger a background save to self-heal local storage
      if (loadedFromGithub) {
        fetch("/api/save_map", {
          method: "POST",
          ...
          if (res.ok) console.log("Self-healed local GeoJSON file successfully.");
    ...
    console.warn("Both local and Github GeoJSON failed. Falling back to Hexagonal Grid map.");
    renderHexagonalMap(container, districtsData, onSelectDistrict);
  }
}
